...
状态：主体已实施；“三行语言前端不在本轮实现”的原始范围后来被 2026-07-19 的用户决定扩展，当前生产代码已实现三行状态与 Xbox App 手动语言目录。
...
- 原始范围不实现 FH6 游戏语言、实际文字语言和语音语言的三行前端；该限制后来已被用户决定取代，当前实现以生产代码和 `docs/DECISIONS.md` 顶部决定为准。
- 不重做主题、字体、卡片视觉或导航宽度。
