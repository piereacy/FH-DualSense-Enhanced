...
- 状态：已完成；四种真实材质均通过 Bluetooth + Forza Data Out 验证
...
- 铺装路面：稳定 `surface_rumble=0`，R2 扳机键输出位于 `90..180 Hz`；`123/124` 个采样匹配，唯一例外是 EWMA 首帧的 rev-limiter 过渡。
- 积水路面：主导驱动轮出现微小正 `wheel_in_puddle` 后优先判为积水，稳定输出位于 `80..150 Hz`。
- 泥土路面：四轮稳定 `surface_rumble=0.120`，`199/199` 个采样位于 `30..70 Hz`。
- 碎石路面：在有效碎石区四轮稳定 `surface_rumble=0.600`，wheelspin 输出覆盖 `12..30 Hz`；离开碎石表面时按实际主导轮切换到其他材质，重新进入后恢复。
- 用户完成四段测试且未报告手感异常。没有生产算法或默认值修改。
