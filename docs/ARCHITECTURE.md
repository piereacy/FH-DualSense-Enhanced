...
`src/modules/forzahorizon/effects.py` 的 `TriggerAnimations` 保存换挡和红线等跨帧 deadline。`Controller` 每个遥测 tick 生成 L2 和 R2 frame，并采用 first-match priority，较高优先级会遮蔽后续效果。

当前 L2 顺序：

1. 换挡冲击。
2. 固定频率和幅度的 ABS pulse。
...
轮胎打滑目前只在达到条件后按路面分类选择固定频率，强度主要来自 `wheelspin_amp`。ABS 目前使用固定 `abs_freq` 和 `abs_amp`，不会保留顶部 wall。EWMA、动态滑移频率、G force damping 和 GT7 风格 ABS wall 均未在当前代码实现。
...
DSX 拥有手柄时，`HapticManager` 明确关闭本项目 body haptics。当前没有 DSX 握把触觉回退路径。
...
- `src/modules/forzahorizon/effects.py` 把已经实现的 idle buzz 注释为 `future enhancement`。
