...
`src/modules/forzahorizon/effects.py` 的 `TriggerAnimations` 保存换挡、红线、动态 wheelspin EWMA、wheelspin hysteresis 和 ABS hold deadline 等跨帧状态。`Controller` 每个遥测 tick 生成 L2 和 R2 frame，并采用 first-match priority，较高优先级会遮蔽后续效果。遥测 `on=False` 时会复位这些 transient state 和两侧 wall latch，防止恢复后沿用旧效果。

当前 L2 顺序：

1. 换挡冲击。
2. GT7 风格 ABS zoned wall：顶部 zone 保持满强度 wall，下部 zone 动态振动。
3. 接近行程末端的 firmware wall。
4. 可选静态刹车 wall。
5. 刹车阻力曲线。
...
`src/modules/haptics/mixer.py` 从同一份 telemetry 计算 engine、路面、rumble strip、积水、轮胎打滑、悬挂、碰撞、换挡和 ABS 的左右能量。
...
Bluetooth 没有使用音频 endpoint。`to_compatible_rumble()` 把左右 `HapticFrame` 下混为 low/high 两个 motor 值，再由 native HID writer 与扳机 frame 原子写入。
...
R2 普通设置只显示 ABS/wheelspin strength 和 sensitivity。threshold、frequency band、EWMA、G damping、hysteresis、burnout 和 ABS wall/hold 参数位于 GUI/TUI 默认折叠的 `EXPERIMENTAL_SECTIONS`，并保持 Profile 级。两套 UI 仍重复声明字段，由 `tests/test_haptic_settings.py` 检查字段顺序、翻译、Profile/share-code round-trip 和 TUI 实际挂载状态。
