...
FH-DualSense-Enhanced `R4` 是基于 `Forza-Horizon-DualSense-Python 1.6.2` 制作、并参考 `HorizonHaptics 1.3.0` 的增强版本。程序读取游戏通过 UDP 输出的车辆遥测数据，让刹车、油门、发动机、路面、轮胎和碰撞状态直接反馈到 DualSense 手柄。
...
- R2 扳机键红线与握把红线互不共用开关；默认关闭 R2 扳机键红线、开启握把红线，并在左握把提供更清晰的非线性断油脉冲和起始冲击。
- 碰撞握把反馈使用方向包络；可选的握把换挡冲击默认关闭，并拥有独立强度和持续时间。
- 可选的涡轮增压阻力、G 力油门阻力、L2/R2 碰撞冲击和松开扳机时的路面纹理均有独立开关，默认关闭，不改变 R3 的既有手感。
- 可选的转速灯带、红线闪烁与挡位 Player LEDs 同样默认关闭，USB 与 Bluetooth 共用同一灯效状态。
- 车辆真正静止且保持怠速时不会无意义地持续振动。
- 原地轰油门和烧胎仍会产生符合车辆状态的反馈。
- 路面材质只在车辆移动或轮胎空转产生实际激励时参与反馈。
- 支持 USB 和 Bluetooth 连接。
- 统一使用 Miku Console 初音未来青绿色界面；驾驶反馈页支持滚轮、嵌套滚动和窄窗口单列布局。
- `Default` 会自动保存并跨重启保留；退出时可另存为命名配置，恢复默认设置会保留已有命名配置。
- 首次启动会按系统显示语言自动选择界面语言，“系统与更新”旁的白点持续提示可用更新。
- Windows 独立 EXE 支持应用内检查、下载、SHA-256 校验、重启安装和失败回滚；ZUV 继续作为兼容与开发入口。
...
2. `src/modules/haptics/pcm.py` 的 `HapticPcmRenderer` 对 USB 和 Bluetooth 使用相同的 65 Hz low、190 Hz composite high、动态 engine saw、相位连续性和 `0.35` block smoothing，因此两条路径表达的是同一波形语义。USB 保留 PCM 安全限幅；Bluetooth 在量化前使用软限幅，减少多层叠加后的硬裁剪失真。
3. USB 以 48 kHz、每块 512 帧生成左右 PCM，并写入 DualSense 四声道音频端点的第 3、4 通道。
4. Bluetooth 以 3 kHz、每块 32 帧生成同一左右 PCM。两者每块时长都是约 10.67 ms；Bluetooth 通过一阶误差反馈把 32 帧量化为 64 字节交错 signed int8 左右采样，尽量保留低幅细节。
...
2. 下载唯一的 Windows 主程序：`FH-DualSense-Enhanced-R4.exe`。
3. 双击 EXE 即可使用；不需要 BAT、ZUV、uv 或本机 Python。设置保存在 EXE 旁边的 `data` 文件夹。
4. “系统与更新”可手动检查、下载并重启安装；自动检查默认开启，后台下载默认关闭。更新器只接受规范名称 `FH-DualSense-Enhanced-R<n>.exe`，并校验 Release 提供的 `.sha256`。

更新替换失败时，独立 Helper 会恢复旧 EXE；程序不会静默请求管理员权限。源码模式、Linux 和 ZUV 模式不会显示可执行的 Windows EXE 自替换操作。

### ZUV 兼容入口

仍可只下载 `win_start.bat`。双击后，启动器会下载 `FH-DualSense-Enhanced.zuv.py`，并自动准备 uv 和独立 Python 环境。

网络不稳定时，可同时下载 `FH-DualSense-Enhanced.zuv.py` 并放在 `win_start.bat` 旁边；启动器会优先使用本地文件。ZUV 不是运行 R4 独立 EXE 的必需组件。

需要试用滚动测试版时，可使用 `R4-preview` Release，并通过 `uv run FH-DualSense-Enhanced.zuv.py --prerelease` 跟踪预发布频道。
...
- The app now uses one Miku Console interface with wheel scrolling, nested-scroll routing, and a responsive one-column driving page for narrow windows.
...
- Miku Console に統一され、ホイールスクロール、入れ子スクロールの振り分け、狭い画面向けの 1 列表示に対応します。
- `Default` は自動保存され、再起動後も保持されます。終了時に名前付きプロファイルへ保存でき、初期設定へ戻しても既存の名前付きプロファイルは保持されます。
- 初回起動時はシステム表示言語を使用し、「システムと更新」の白い点が利用可能な更新を継続して知らせます。
...
2. Windows 主プログラム `FH-DualSense-Enhanced-R4.exe` をダウンロードします。
3. EXE を直接実行します。Python、BAT、ZUV、uv は不要で、設定は隣の `data` フォルダーに保存されます。
4. 「システムと更新」から確認、ダウンロード、検証、再起動インストールを実行できます。自動確認は既定で有効、バックグラウンドダウンロードは無効で、`FH-DualSense-Enhanced-R<n>.exe` だけを更新対象として受け付けます。
