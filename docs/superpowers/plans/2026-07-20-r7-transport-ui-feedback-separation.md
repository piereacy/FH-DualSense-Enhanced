...
- 删除 `src/modules/dualsense/main.py` 中错误的两个 `0x20` 常量、USB audio-haptics 状态和 setter。
- 让普通 USB/BT `_build()` 恢复 R6 规则：L2/R2 扳机键字段始终按既有逻辑输出，compatible rumble 仅在显式提供时声明，visual 字段继续保持 R7 行为。
- 删除 `src/modules/haptics/manager.py` 中所有 HID 音频模式请求；保留 USB PCM、Bluetooth `0x36`、失败回退和重试生命周期。
- 删除断言错误 `0x20` 或单次 `0x01` 重置的测试，新增 R6 HID 字节契约和“transport 切换不写协议模式位”回归测试。
- 不修改候选预验证、原子 handover、USB 优先、PortAudio refresh、callback 健康检查或反馈算法。
...
- 重启或彻底断电清除旧候选残留状态后，实机依次验证 R6 干净基线、候选 USB/Bluetooth 冷启动、退出候选后再次运行 R6、BT -> USB、USB -> BT、20 分钟握把触觉续航与窗口缩放；记录连接方式、Steam Input 和游戏内振动状态。
