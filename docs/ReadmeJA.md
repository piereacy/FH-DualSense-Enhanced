...
- 任意のコントローラーライトとシンプルな Miku Console UI を備えています。
...
テレメトリ触覚はゲーム内の振動設定に依存しません。ゲーム振動を有効にするとメニューやカットシーンの振動を維持できますが、衝突方向が分かりにくくなる場合があります。方向や重複効果を比較するときだけ、一時的に無効にしてください。
