...
GUI 总览由 `src/modules/gui/overview_status.py` 负责纯状态映射，`OverviewTab` 只更新控件。控制器事实来自当前 native/DSX backend 与持久化错误字段；Profile 来自配置存储；更新状态来自 `UpdateService` 的不可变快照；遥测状态来自 `UDPListener.snapshot()`；Xbox App 输入状态来自 `XInputBridgeService.snapshot()`。页面构造完成后立即渲染一次，之后由 GUI 每秒状态 tick 刷新，设置或语言切换仍可通过已有 refresh callback 立即触发。短状态和详细提示分开，异常原因不占用 H1 主文案。
...
`src/modules/dualsense/input_state.py` 只解析已验证长度和 report ID 的 USB/BT 输入报告，生成不可变 `DualSenseInputState`。`src/modules/dualsense/main.py` 的 I/O thread 是物理 HID 的唯一 reader；设置 input consumer 后，它只把最新解析状态和单调时钟时间非阻塞发布给 bridge，不执行 ViGEm 调用。Steam 模式或 DSX backend 不挂 consumer。

`src/modules/xinput/report.py` 把标准按钮、D-pad、摇杆和 L2/R2 扳机键映射为 `XUSB_REPORT`，不增加 deadzone、平滑或 response curve。`bridge.py` 的单 worker 独占 `ViGEmClient` 和虚拟 Xbox 360 target，只保留 latest state：100 ms 未收到有效报告时发送一次全中立，3 s 时移除 target；恢复时先重建并中立化，不回放 stop 前状态。`service.py` 负责 `preferred_forza_platform` 的生命周期切换，Steam 模式先解除 consumer、停止 worker并移除 target。
...
- 可选 input consumer 也由同一个 I/O thread 驱动；USB/BT 报告分别按协议解析后只发布 latest state。不得增加第二个 HID reader，也不得让 ViGEm worker 写物理 DualSense handle。
...
- XInput bridge 的单 HID reader、latest-only、100 ms 中立、3 s target 移除和 Steam 模式彻底停用是输入安全边界；不得为了吞吐改成排队回放，也不得让 platform UI 直接操作 ViGEm target。
