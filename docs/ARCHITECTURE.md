...
所有幅度最终通过 `clamp01()` 限制到 `0..1`。`CompatibleRumble` 只有 low/high 两个通道，用于 Bluetooth HID 报告。可选 compatible 字段只由需要保留侧别的高优先级事件填写；普通连续反馈不填写，继续走原有按频率下混。
...
- 握把红线默认关闭。启用后默认只在左握把输出 10 Hz、50% duty、`192/255` high 加 `25%` low 的断油脉冲，并在最终限幅前应用 `1.5` 信号增益。左右握把可独立启用，握把红线使用独立的 `enable_grip_redline_haptics`，不受 R2 扳机键 `enable_rev_limiter` 控制。
...
Bluetooth 没有使用音频 endpoint。普通 `HapticFrame` 由 `to_compatible_rumble()` 继续按频率下混：左右 low 取较强值进入 low-frequency motor，左右 high 取较强值进入 high-frequency motor。这样保留连续路面材质的既有频率语义。

红线和碰撞属于需要保留左右侧别的高优先级事件。`HapticMixer` 在这些事件 active 时显式计算 `compatible_low_frequency` 和 `compatible_high_frequency`，把左侧事件投影到 low-frequency motor、右侧事件投影到 high-frequency motor，同时混入已经压低的连续背景。这里的 low/high motor 被用作 Bluetooth 的左右空间代理，不改变 USB 四通道 frame。USB 与 Bluetooth 的事件检测、时序和归一化强度仍来自同一份 mixer 状态，代码不定义强弱或高低配行为。
...
R3 普通设置分别显示 R2 扳机键红线、握把红线、traction/grip、R2 扳机键换挡和握把换挡的常用参数；控制页把扳机红线、握把红线和握把换挡开关分开，并提供左握把、右握把选择。握把换挡开关、强度和持续时间属于普通设置。握把红线 gain、release、low ratio、背景压低以及碰撞包络/冷却/侧别参数位于 GUI/TUI 默认折叠的 `EXPERIMENTAL_SECTIONS`。threshold、frequency band、EWMA、G damping、hysteresis、burnout 和 ABS wall/hold 也保持在该区域，全部属于 Profile。两套 UI 仍重复声明字段，由 `tests/test_haptic_settings.py` 检查字段顺序、翻译、Profile/share-code round-trip 和 TUI 实际挂载状态。
...
- R2 命名 Profile 保留原 `rev_limit_*` 扳机参数，并获得默认关闭的握把红线、`1.5` 红线增益和默认关闭的握把换挡字段。
