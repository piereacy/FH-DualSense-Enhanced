...
- version 3 且缺少新握把字段：把当前 `enable_rev_limiter` 复制为握把总开关初始值。若 `rev_limit_freq/amp` 恰好是已知预发布默认 `10/96`，握把改用本轮新默认 `10/192`，扳机恢复为 `30/12`；其他自定义值继续保留为扳机值，同时复制给握把，避免静默丢失用户调校。
