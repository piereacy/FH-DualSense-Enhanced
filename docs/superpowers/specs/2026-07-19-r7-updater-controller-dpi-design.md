...
日期：2026-07-19
状态：生产代码、自动测试、冻结构建与真实 R6 到 R7 隔离升级已实现；真实 DualSense 和混合 DPI 视觉验收待完成
...
- 新版健康后静默清理同目录全部严格命名且版本更低的规范 EXE，以及对应 `.exe.old` 和 `.exe.sha256` 遗留；先逐个迁移旧 EXE 快捷方式，不执行任意 `*.exe`/`*.old` 通配删除。
...
5. 确认成功后枚举同目录全部严格命名且版本更低的规范 EXE，逐个迁移快捷方式。
6. 没有快捷方式或迁移成功的旧规范 EXE全部删除，并清理严格同名的 `.exe.old`、`.exe.sha256`；某一旧版部分迁移失败时只暂时保留该规范 EXE并记录待修复状态。
