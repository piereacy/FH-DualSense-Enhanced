...
Enhanced R5 的 GUI 总览由 `src/modules/gui/overview_status.py` 负责纯状态映射，`OverviewTab` 只更新控件。控制器事实来自当前 native/DSX backend 与持久化错误字段；Profile 来自配置存储；更新状态来自 `UpdateService` 的不可变快照；遥测状态来自 `UDPListener.snapshot()`。页面构造完成后立即渲染一次，之后由 GUI 每秒状态 tick 刷新，设置或语言切换仍可通过已有 refresh callback 立即触发。短状态和详细提示分开，异常原因不占用 H1 主文案。

总览快捷入口还包含一个跨两列的“启动 FH6”按钮。`OverviewTab` 在后台复用 Windows Steam 安装发现并缓存验证后的根目录，每秒用精确进程名刷新运行状态，未发现时最多每五秒重新扫描一次；扫描、启动请求和路径持久化都不阻塞 Tk 主线程。按钮只在安装有效且游戏未运行时启用，点击后 `launch_fh6_via_steam()` 再次验证安装与进程状态，并通过 `steam://run/2483190` 交给 Steam 启动。请求发出后最多保持二十秒“正在启动”状态；一旦检测到游戏进程即显示“FH6 运行中”并禁用。此入口不直启 EXE，也不读取、交换或修复语言 ZIP。
...
### 3.4 Windows Steam FH6 语言包按钮

`src/modules/forzahorizon/fh6_language.py` 是独立于遥测与手柄后端的 Windows Steam 工具层。安装发现按 Steam 注册表根目录、每个根目录的 `steamapps/libraryfolders.vdf`、App ID `2483190` 的 `appmanifest_2483190.acf`、卸载注册表和精确名称 `ForzaHorizon6.exe` 的完整进程路径逐级尝试，最后才使用已验证的 global 缓存或用户手动选择。验证要求游戏根目录同时存在 `ForzaHorizon6.exe` 与 `media/Stripped/StringTables`；生产代码不假定盘符或 `Program Files`。

语言状态不是内存布尔值，而是每次从 `CHS.zip`、`EN.zip` 和 `CHS.zip.fhds-swap.tmp` 的实际存在性与 ZIP 内多个 `.str` UTF-8 样本判定。可识别状态为原始、已交换、可恢复的中断状态、缺失、未知和损坏。只有一份可识别中文包和一份可识别英文包时才允许动作；Steam manifest 明确不是 English 时禁止启用，无法读取时必须额外确认。普通状态轮询、启动扫描和手动选目录全部只读。

启用与还原共用三步 rename：`CHS.zip -> temp`、`EN.zip -> CHS.zip`、`temp -> EN.zip`。每一步前重新验证路径、源/目标存在性和 FH6 未运行，异常时按已完成步骤逆序回滚。进程崩溃留下 temp 时，下次启动只报告恢复需求；`repair_native_language()` 只有在用户再次确认且两份内容可识别时才把中文放回 CHS、英文放回 EN，不自动删除任何文件。GUI 使用确认弹窗并在 worker thread 执行，TUI 使用十秒内二次按键确认；两者共享 `fh6_language_presentation.py` 的状态和动作映射。缓存字段 `fh6_install_path` 属于 `preferences.GLOBAL_FIELDS`，不进入 Profile 或 share code。
...
- 通用 `ProcessWatcher` 仍按 `forza` 子串匹配，存在误匹配其他进程的可能；FH6 语言工具改用同一模块的精确 `ForzaHorizon6.exe` 查询与完整路径，不受该子串规则影响。
