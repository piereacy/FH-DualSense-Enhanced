...
- Xbox App 启动仍由用户显式点击。程序先用 `Get-StartApps` 动态查找当前用户已注册的 `PackageFamilyName!Application` AUMID，并通过 `shell:AppsFolder\\<AUMID>` 激活；未找到已安装身份时打开该代游戏的固定 `msxbox://game/?productId=<id>` 产品页，让用户在 Xbox App 中安装或点击“开始游戏”。不直启受保护的游戏 EXE，也不把 URI 已提交写成游戏已进入主菜单。
...
- Xbox App：显示同一显式启动按钮和 bridge 状态。已安装游戏优先按动态 AUMID 激活；无法发现 AUMID 时打开对应 Xbox App 产品页。平台切换不会自动启动游戏。
...
实施结果：包含三行语言状态与 Xbox App 手动语言目录补丁的最终本地 R6 EXE 为 `51,814,190` 字节（`49.414 MiB`），相对线上 R5 增加 `4,595,998` 字节（`4.383 MiB`，`9.73%`），未触发 `52 MiB` 停止线。`--help` 冷启动约 `3.19 s`、紧接的温启动约 `2.04 s`；可见 Xbox App 模式下两段式 PyInstaller 进程三秒合计消耗约 `0.1875 CPU s`，相当于单核 `6.25%`，Steam 同条件差异尚未取得可靠样本。
...
- Xbox App 启动覆盖 `Get-StartApps` JSON 解析、精确代际匹配、拒绝同名 Steam/desktop shortcut、AUMID 直接激活和 product-ID fallback。
...
- Xbox App 游戏安装路径自动发现、安装状态管理、修复或商店管理；启动只限动态 AUMID 激活与固定产品页 fallback。
