...
- 测试使用 `pytest`，发布使用 ZUV、PyInstaller 和 GitHub Actions；Windows 构建一次生成三种 GUI 方案。
...
构建 Windows EXE（会生成 Miku Console、Stage、Studio 三个主程序、各自 `.sha256` 和更新 Helper）：
...
- 三种 GUI 必须实例化同一套 Tab、`Settings` 和后端。布局元数据只放在 `src/modules/gui/variants.py`，主题颜色只放在 `src/modules/gui/theme.py`；不得复制业务页面或为某个 EXE 建立独有功能。
- `FHDS_BUILD_VARIANT` 仅供 Windows 打包，源码预览可用 `FHDS_UI_VARIANT=console|stage|studio`。冻结 EXE 必须读取内置 `data/ui_variant.txt`，更新器也必须按当前方案选择同名资产。
- 内置自更新只允许冻结后的 Windows 独立 EXE。必须保留稳定 `R<n>` tag、同方案 EXE、配套 `.sha256`、MZ 头检查、`.part` 下载、独立 Helper 和 `.old` 回滚；源码、Linux 和 ZUV 运行不得尝试覆盖可执行文件。
...
8. 涉及 Windows R4 打包时，确认三个 EXE 均存在、哈希文件匹配、版本资源为当前 `R<n>`，并分别执行启动冒烟测试；只验证其中一个不算完成。
