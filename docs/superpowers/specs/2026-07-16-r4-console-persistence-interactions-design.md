...
Enhanced R4 当时的生产代码包含 Console、Stage 和 Studio 三种构建时 GUI 壳层。用户完成对比后选择 Console 作为唯一正式前端，Stage 和 Studio 只用于方案评审，不进入最终产品。
...
- R4 只保留一个 Console GUI 和一个 Windows EXE。
...
R4 尚未公开发布，因此不提供 Stage 或 Studio 的更新兼容层，也不存在已发布 R4 用户被跨方案替换的问题。
