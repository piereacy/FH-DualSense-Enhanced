...
- Miku Console、Miku Stage、Miku Studio 三套初音未来青绿色界面共用完全相同的后端、页面、设置和配置文件。
...
2. 按喜欢的布局下载下面任意一个文件。三者功能和配置格式完全一致：
   - `FH-DualSense-Enhanced-R4-Miku-Console.exe`：完整左侧文字导航，最稳妥、适合较窄窗口。
   - `FH-DualSense-Enhanced-R4-Miku-Stage.exe`：顶部横向导航，内容区更宽。
   - `FH-DualSense-Enhanced-R4-Miku-Studio.exe`：紧凑导航轨与工作台式布局，适合集中调参。
3. 双击 EXE 即可使用；不需要 BAT、ZUV、uv 或本机 Python。设置保存在 EXE 旁边的 `data` 文件夹。
4. “系统与更新”可手动检查、下载并重启安装；自动检查默认开启，后台下载默认关闭。更新器只安装与当前 Miku 界面方案同名的资产，并校验 Release 提供的 `.sha256`。
