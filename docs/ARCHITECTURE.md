...
- R2 扳机键红线默认关闭，握把红线默认开启。握把侧默认只在左握把输出 10 Hz、50% duty、`192/255` high 加 `25%` low 的断油脉冲，并在最终限幅前应用 `1.5` 信号增益。左右握把可独立启用，握把红线使用独立的 `enable_grip_redline_haptics`，不受 R2 扳机键 `enable_rev_limiter` 控制。
...
Bluetooth 不依赖 Windows 音频 endpoint。`src/modules/haptics/bt_audio.py` 以 3 kHz、每块 32 帧调用与 USB 相同的 `HapticPcmRenderer`。USB 的 512 / 48000 与 Bluetooth 的 32 / 3000 都是约 10.667 ms，因此低频、高频、engine、左右混音和 `0.35` 平滑位于相同时间尺度。PCM 被量化成 64 字节交错 signed int8，并约每 10.667 ms 发送一次。
...
- R2 命名 Profile 保留原 `rev_limit_*` 扳机参数；缺失的握把字段按当前默认值补齐，因此握把红线默认开启并获得 `1.5` 红线增益，握把换挡仍默认关闭。已经显式保存新字段的命名 Profile 不会被覆盖。
