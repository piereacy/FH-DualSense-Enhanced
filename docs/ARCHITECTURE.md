...
Enhanced R4 普通设置分别显示 R2 扳机键红线、握把红线、traction/grip、R2 扳机键换挡和握把换挡的常用参数；控制页把各开关分开，并提供左右握把选择。握把换挡开关、强度和持续时间属于普通设置。握把红线曲线、起始冲击、碰撞包络、增压/G 力阻力、碰撞扳机冲击、空闲扳机纹理以及 traction/ABS 的低层参数位于 GUI/TUI 默认折叠的实验性区域，全部属于 Profile。灯效有独立页面，转速灯带与挡位 Player LEDs 默认关闭。两套 UI 仍重复声明字段，由设置、翻译、Profile/share-code 和 TUI 挂载测试保证一致。
