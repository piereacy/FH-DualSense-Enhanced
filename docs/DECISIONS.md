...
- 状态：比较口径和老三样约束已确认；具体 README 功能清单等待与用户确认后再写入。
...
## 2026-07-17：README 不宣传界面设计代号，并要求关闭游戏内振动

- 状态：三语 README、文档契约和 GitHub `main` 已更新；应用界面和 R4 功能代码未修改。
- 命名：用户仍要求保留现有 Miku Console 青绿色视觉设计。本决定只禁止在 README 把 “Miku Console” 当作产品概念或功能卖点，不授权删除主题、布局或程序内现有设计。
