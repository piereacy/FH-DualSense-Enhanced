...
- 当前锁定的 `sounddevice` 实现在模块导入末尾直接调用 `_initialize()`，因此 PortAudio 设备快照早在应用启动时就可能建立，而不是等第一次 `query_devices()`。

这组证据否定了“仅发送 `0x08 / 0x02` 就能恢复 USB 握把触觉”的假设。当前最符合代码和实机现象的原因是：应用在 Bluetooth 阶段导入 `sounddevice` 并初始化 PortAudio，此时 Windows 尚未发布 USB 音频端点；端点稍后出现，但该进程中的设备表没有随系统状态更新。
...
2. 在导入 `sounddevice` 或初始化 PortAudio 之前，用不依赖 PortAudio 的 Windows 端点探针确认 DualSense USB render endpoint 已活动。
...
同一手柄的 USB candidate 稳定出现后先保留当前 BT handle。经过 3 秒稳定窗口后，通过 Windows MMDevices render endpoint 状态进行只读判断；只有端点活动时才继续现有 USB candidate validation、Bluetooth control teardown 和原子 handover。默认 USB audio backend 在这个边界之前不得导入 `sounddevice`。

优点是 PortAudio 的首次初始化发生在系统端点已经出现之后，并且失败时不会牺牲正在工作的 Bluetooth 握把触觉。探针使用 Python 标准库，不增加 EXE 第三方依赖。
...
`src/modules/haptics/audio.py` 当前在模块导入期同时导入 NumPy 与 `sounddevice`。该行为必须改为默认 backend 的延迟加载：

- 导入 `modules.haptics`、构造 `UsbAudioHaptics` 或在 Bluetooth 模式运行都不得导入 `sounddevice`。
- 只有 `UsbAudioHaptics.start()` 在 USB eligibility 已满足后才加载默认 `sounddevice` module；`sounddevice` 随后的 PortAudio 初始化仍由依赖本身完成。
- 单元测试传入的 fake sounddevice/NumPy module 继续直接使用，不触发真实依赖加载。
- 依赖缺失时继续返回 `False` 并使用现有 warning 语义。
- `OutputStream` 的 device、48 kHz、4 channel、float32、blocksize 512、callback、PCM channel 2/3 映射和 stop 行为全部保持不变。

这是对 R6 USB audio lifecycle 的唯一允许改动。不得借延迟加载重新加入私有 `_terminate()`、`_initialize()`、额外 lifecycle lock、heartbeat 或周期性 PortAudio refresh。
...
2. deadline 之前的 topology scan 继续保留 Bluetooth，并且不调用 readiness callback、不导入 `sounddevice` 或初始化 PortAudio。
...
7. USB commit 后，现有 `UsbAudioLifecycle` 第一次调用 `UsbAudioHaptics.start()`；此时才延迟导入 `sounddevice`，由依赖初始化 PortAudio，再调用 `query_hostapis()` 与 `query_devices()`。
...
9. 导入 `modules.haptics` 和构造默认 `UsbAudioHaptics` 不加载真实 `sounddevice`；首次 USB `start()` 才加载一次，fake backend 不加载真实依赖。
10. 现有 PortAudio 私有 API 禁用契约继续通过；`audio.py` 仅改变默认依赖加载时机，`lifecycle.py` 和 `manager.py` 不加入新的 refresh、heartbeat 或 retry 机制。
...
- Enhanced R6 的 USB stream 参数、PCM channel 2/3 映射、callback 与 stop 行为；`audio.py` 只允许改变默认 `sounddevice` 的导入时机。
- GUI/TUI 页面、DPI、更新器、FH6 utilities、XInput bridge 和发布版本号。
- 不新增用户设置；3 秒窗口属于 transport 正确性交接。

## 成功标准

- 自动测试证明真实 `sounddevice` 不会在 endpoint readiness 之前导入，PortAudio 不会提前初始化。
